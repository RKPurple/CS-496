...
and
  ...
  | Proj(e,id) ->
    eval_expr e >>= fields_of_recordVal >>= fun x ->
      (match List.assoc_opt id x with
      |None -> error "field not found"
      |Some (true, RefVal(v)) -> Store.deref g_store v
      |Some (false, v) -> return v
      |_ -> error "Immutable Field")
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=  fields_of_recordVal >>= fun v ->
      (match List.assoc_opt id v with
      |None -> error "field not found"
      |Some (false, _) -> error "Immutable field"
      |Some (true, RefVal(x)) ->
        eval_expr e2 >>= 
        Store.set_ref g_store x >>= fun _ -> return UnitVal
      |Some (true, _) -> error "Expected a reference")
  | IsNumber(e) ->
    eval_expr e >>= fun v ->
      let result =
        match v with
